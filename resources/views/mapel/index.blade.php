                ...
                <div class="card border-0 shadow rounded">
                    <div class="card-body">
                        ...
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                @forelse ($mapel as $index => $mapel1)
                                    <tr>
                                        <td>{{ $mapel->firstItem() + $index }}</td>
                                        <td>{{ $mapel1->nama_mapel }}</td>
                                        <td class="text-left">
                                            <form onsubmit="return confirm('Apakah Anda Yakin ?');" action="{{ route('mapel.destroy', $mapel1->id) }}" method="POST">
                                                <a href="{{ route('mapel.edit', $mapel1->id) }}" class="btn btn-sm btn-primary">EDIT</a>
                                                @csrf
                                                @method('DELETE')
